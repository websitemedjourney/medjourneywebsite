  ...
  useEffect(() => {
    const fetchYoutubeData = async () => {
      try {
        ...
        if (!apiKey) {
          console.warn("Youtube API Key is missing. Falling back to static data.");
        ...
        if (!channelId) {
          console.warn("Youtube Channel ID is missing. Falling back to static data.");
          setData(configData as YoutubeData);
          return;
        }
        ...
        if (searchData.items && searchData.items.length > 0) {
          ...
          const decodedTitle = tempDiv.textContent || tempDiv.innerText || video.snippet.title;
    ...
    <section id="video" className="section-pad theme-bg">
      <div className="container-px">
        <div className="text-center mb-10">
          ...
          {data.channelName && (
            <p className="text-muted-foreground mt-2">From the channel of {data.channelName}</p>
          )}
        </div>
